...
class Analyzer:
    def __init__(self, schedule_obj, machine_obj, init_mapping, timingFileMode=False, timingsFileName=None):
        self.schedule = schedule_obj
        self.machine = machine_obj
        self.init_map = init_mapping #here is where we can modify initial mapping

        self.honeywell_mode = True
        self.chain_heating = []
        self.qubit_heating_quantas = {}
        self.timing_file = timingsFileName
        self.timing_file_mode = timingFileMode
    ...
    def gate_fidelity(self, sys_state, trap_id, ion1, ion2):
        ...
        #print("ion 1", ion1)
        #print("ion 2", ion2)
        #print("trap id", trap_id)
        #print("sys state", sys_state.print_state())
        #print("chain heating array", self.chain_heating)
    ...
    def move_check(self): #if I want to get custom fidelity for cyclone, making a custom schedule is easiest
        op_count = {}
        ...
        print("Parallelization saved:", op_times[Schedule.Gate] + op_times[Schedule.Split] + op_times[Schedule.Move] + op_times[Schedule.Merge] - prog_fin_time)
        print("Now trying to dump timings if mode is on")
        if (self.timing_file_mode):
            timings_file_name = self.timing_file
            print("Dumping timings file here")
            print("The file name is", timings_file_name)
            with open(timings_file_name, 'wb') as file:
                pkl.dump(self.schedule.events, file)
