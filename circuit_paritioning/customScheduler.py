import sys
sys.path.insert(0, 'C:/Users/sahil/OneDrive/Desktop/Quantum Computing/QLDPC project/circuit-paritioning/')
#import partitioning
...
def get_custom_mapping(circuit_file, machine, machString):
    ...
    if (machString == "G2x3"):
        ...
        while (i >= 0):
            ...
            i -= 1
        sets = new_sets
        fill_order = [0,1,2,3,4,5] #pack together
...
def countAncilla(circuit_file_name):
    try:
        arr = circuit_file_name.split("Ancilla") #this works as long as there's not 100 ancilla
        if (arr[0][-2] == "e"):
            ancilla_count = arr[0][-1]
        else:
            ancilla_count = arr[0][-1] + arr[0][-2]
        return int(ancilla_count)
    except:
...
def getAncillaBits(circuit_file): #this probably doesn't work if the ancilla is ten
    ...
    for x in circ.get_instructions("cx"):
        ancilla = circ.find_bit(x.qubits[1])[0] #this should be safe
        if (ancilla not in ancilla_list):
